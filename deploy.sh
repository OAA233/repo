...
[ -d source ] && cp -R source .dist/   # GPL 要求随二进制公开的对应源码
[ -d sponsor ] && cp -R sponsor .dist/  # 赞赏页（微信/支付宝收款码）
...
for f in Packages Packages.zst Packages.bz2 Packages.gz Release index.html CydiaIcon.png icons/*.png depictions/*.json pkg/*.html shots/*/* mac/* source/* sponsor/*; do
